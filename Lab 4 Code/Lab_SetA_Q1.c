...
#define TOTAL_DISK_BLOCKS 64
#define TOTAL_DISK_INODES 16
#define BLOCK_SIZE 1
...
void main()
{
    ...
    printf("Size (kB) of each block: %.1f\n", (float) BLOCK_SIZE/2);
    ...
    for(i = 0; i < numFiles; i++)
    {
        ...
        fileTable[i] = newFile;
        ret = AllocateBlocks(2 * fileTable[i].fileSize);
        ...
        if(ret == 0)
        {
            ...
            for(j = blockStart ; j < blockStart + (2 * newFile.fileSize); j++)
            {
                blockStatus[j] = 1;
            }
    ...
    for(i = 0; i < numFiles; i++)
    {
        ...
        for(j = 0; j < (2 * fileTable[i].fileSize ); ++j)
        {
            if(j < ( 2 * fileTable[i].fileSize -1) )
            {
